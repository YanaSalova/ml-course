...
class KMeans:
    ...
    def fit(self, X: np.array, y=None) -> NoReturn:
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(self.X.shape[0]):
                labels.append(
                    np.argmin(np.linalg.norm(self.X[i] - self.centroids, axis=1))
                )
            labels = np.array(labels)

            for i_cluster in range(self.n_clusters):
                if np.sum(labels == i_cluster) > 0:
                    self.centroids[i_cluster] = np.mean(
                        self.X[labels == i_cluster], axis=0
                    )
                else:
                    self.centroids[i_cluster] = self.X[
                        np.random.choice(self.X.shape[0])
                    ]
    ...
    def predict(self, X: np.array) -> np.array:
        ...
        for cluster_ind in range(self.n_clusters):
            distances[:, cluster_ind] = np.linalg.norm(
                X - self.centroids[cluster_ind], axis=1
            )
...
class AgglomerativeClustering:
    ...
    def update_dist(
        self, dist_matrix, clusters, upd_cluster, del_cluster, another_cluster
    ):
        if self.linkage == "average":
            ...
            dist_matrix[upd_cluster][another_cluster] = (
                power_upd_cluster * dist_matrix[upd_cluster][another_cluster]
                + power_del_cluster * dist_matrix[del_cluster][another_cluster]
            ) / (power_upd_cluster + power_del_cluster)
            dist_matrix[another_cluster][upd_cluster] = dist_matrix[upd_cluster][
                another_cluster
            ]

        elif self.linkage == "single":
            dist_matrix[upd_cluster][another_cluster] = np.minimum(
                dist_matrix[upd_cluster][another_cluster],
                dist_matrix[del_cluster][another_cluster],
            )
            dist_matrix[another_cluster][upd_cluster] = dist_matrix[upd_cluster][
                another_cluster
            ]
            ...
            dist_matrix[upd_cluster][another_cluster] = np.maximum(
                dist_matrix[upd_cluster][another_cluster],
                dist_matrix[del_cluster][another_cluster],
            )
            dist_matrix[another_cluster][upd_cluster] = dist_matrix[upd_cluster][
